...
#### ビル・ゲイツが映っている画像から顔を抽出するプログラム。（学習データ/テストデータ用）
#### 複数人映っている場合、この段階ではどれがビル・ゲイツかは判別できず、抽出した画像からビル・ゲイツの画像のみを取捨選択する必要がある。
...
# 集めてきた画像データのあるディレクトリ
input_data_path = './billgates_images/billgates'
# 切り抜いた画像の保存先ディレクトリ
save_path = './cutted_bill_images/'
...
for i in range(image_count):
  ...
  if len(face) > 0:
    for rect in face:
      ...
      w = rect[2]
      h = rect[3]
      cv2.imwrite(save_path + 'cutted_bill' + str(face_detect_count) + '.jpg', img[y:y+h, x:x+w])
